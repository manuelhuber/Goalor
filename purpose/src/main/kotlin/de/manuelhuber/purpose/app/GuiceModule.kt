...
class GuiceModule : KotlinModule() {
    override fun configure() {
        ...
        bindConstant().annotatedWith(Names.named(DB_URL))
            .to(System.getenv(DB_URL))
        bindConstant().annotatedWith(Names.named(DB_PASSWORD))
            .to(System.getenv(DB_PASSWORD))
        bindConstant().annotatedWith(Names.named(DB_USER))
            .to(System.getenv(DB_USER))
        bindConstant().annotatedWith(Names.named(STATIC_FILE_FOLDER))
            .to(System.getenv(STATIC_FILE_FOLDER))
        bindConstant().annotatedWith(Names.named(QUEUE_HOST))
            .to(System.getenv(QUEUE_HOST))
